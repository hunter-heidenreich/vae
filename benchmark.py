"""
Benchmark script for VAE forward pass with different KL and sampling configurations.

Compares the four combinations of:
- use_analytic_kl: True/False
- use_distributions: True/False
...
def benchmark_forward_pass(
    ...
    Args:
        ...
        use_analytic_kl: Whether to use analytic KL divergence
...
def run_benchmark_suite(
    ...
    configs = list(
        product(
            ...
            latent_dims,
            batch_sizes,
            [True, False],  # use_analytic_kl
            [True, False],  # use_distributions
        )
    )
...
def print_summary(df: pd.DataFrame):
    ...
    summary = (
        df.groupby(["use_analytic_kl", "use_distributions"])
        .agg(
            {
                "mean_ms": ["mean", "std"],
                "median_ms": "mean",
    ...
    best_config = df.groupby(["use_analytic_kl", "use_distributions"])["mean_ms"].mean()
    best = best_config.idxmin()
    print(f"use_analytic_kl={best[0]}, use_distributions={best[1]}")
    print(f"Average time: {best_config[best]:.3f} ms")

    # Breakdown by dimension type
    print("\n--- Performance vs Hidden Dimension ---")
    hidden_analysis = (
        df.groupby(["hidden_dim", "use_analytic_kl", "use_distributions"])["mean_ms"]
        .mean()
        .unstack([1, 2])
    )
    ...
    latent_analysis = (
        df.groupby(["latent_dim", "use_analytic_kl", "use_distributions"])["mean_ms"]
        .mean()
        .unstack([1, 2])
    )
    ...
    batch_analysis = (
        df.groupby(["batch_size", "use_analytic_kl", "use_distributions"])["mean_ms"]
        .mean()
        .unstack([1, 2])
    )
