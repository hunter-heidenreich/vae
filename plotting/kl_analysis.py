...
def save_kl_diagnostics_separate(test_history: dict, fig_dir: str, active_threshold: float = 0.1):
    ...
    fig, ax = plt.subplots(figsize=(12, 8))
    _plot_active_units_over_time_standalone(ax, epochs, active_units_over_time, latent_dim)
    plt.tight_layout()
...
def _plot_kl_bar_chart_standalone(ax, kl_matrix, latest_kl_per_dim, active_threshold):
    ...
    ax.axhline(y=active_threshold, color="red", linestyle="--", alpha=0.7, 
    ...
    if latent_dim <= 20:
        for bar, val in zip(bars, latest_kl_per_dim):
            ...
            ax.text(bar.get_x() + bar.get_width() / 2.0, height + max(latest_kl_per_dim) * 0.01,
                   f"{val:.2f}", ha="center", va="bottom", fontsize=8)


def _plot_active_units_over_time_standalone(ax, epochs, active_units_over_time, latent_dim):
    """Plot active units over time as standalone plot."""
    ax.plot(epochs, active_units_over_time, "o-", color="steelblue", linewidth=2, markersize=4, 
           label="Active Units")
...
def _plot_kl_distribution_histogram_standalone(ax, epochs, kl_matrix, active_threshold):
    ...
    if n_epochs >= 3:
        ...
        for idx, label, color in zip(epoch_indices, epoch_labels, colors_hist):
            ax.hist(kl_matrix[idx], bins=15, alpha=0.6, label=f"{label} (E{int(epochs[idx])})",
                   color=color, edgecolor="black", linewidth=0.5)
    else:
        ax.hist(kl_matrix[-1], bins=15, alpha=0.7, label=f"Final (E{int(epochs[-1])})",
               color="red", edgecolor="black", linewidth=0.5)
    
    ax.axvline(x=active_threshold, color="red", linestyle="--", alpha=0.8, 
              label=f"Threshold = {active_threshold}")
...
def _plot_cumulative_kl_contribution_standalone(ax, latest_kl_per_dim, latent_dim):
    ...
    ax.plot(range(1, latent_dim + 1), cumsum_percentage, "o-", color="purple", 
