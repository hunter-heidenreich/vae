...
def save_latent_scatter(Z: np.ndarray, Y: np.ndarray, out_path: str):
    """Save a visualization of latent representations colored by class labels."""
    if Z.shape[0] == 0:
        raise ValueError("Empty latent array provided")

    with figure_context((6, 5), out_path):
        if Z.shape[1] == 1:
...
def save_latent_histograms(Z: np.ndarray, out_path_2d: str, out_path_1d: str):
    """Save latent distribution visualizations for arbitrary dimensions."""
    latent_dim = Z.shape[1]

    # KDE plot of all marginals
    with figure_context((8, 5), out_path_1d):
    ...
    # 2D visualization based on dimensionality
    if latent_dim == 1:
        with figure_context((6, 4), out_path_2d):
            sns.histplot(
                Z[:, 0], kde=True, stat="density", alpha=0.7, color="steelblue"
            )
            plt.xlabel("z1")
            plt.ylabel("Density")
        ...
        with figure_context((6, 5), out_path_2d):
...
def save_interpolation_figure(
    model: VAE, device: torch.device, out_path: str, n: int = 15
):
    """Create interpolation figure by sweeping along first latent dimension."""
    with model_inference(model):
        z1 = torch.linspace(-3, 3, n, device=device)
        z = torch.zeros(n, model.latent_dim, device=device)
        z[:, 0] = z1
        samples = decode_samples(model, z)

    img = grid_from_images(samples, 1, n)

    with figure_context((n, 1), out_path):
        plt.axis("off")
        plt.imshow(img, cmap="gray")
...
def save_interpolation_between_examples(
    model: VAE,
    loader: DataLoader,
    device: torch.device,
    out_path: str,
    ...
    """Pick two random datapoints, interpolate in latent space, decode, and save."""
    ...
    with model_inference(model):
        ...
        zs = _slerp(z0, z1, t) if method.lower() == "slerp" else _lerp(z0, z1, t)
        imgs = decode_samples(model, zs)

    grid = grid_from_images(imgs, 1, steps)

    with figure_context((steps, 1), out_path):
        plt.axis("off")
...
def save_gradient_norms(train_history: dict, out_path: str):
    """Save gradient norm curve from training history (epoch-based). DEPRECATED - use save_gradient_diagnostics instead."""
    # For backward compatibility, fallback to old behavior if new keys not available
    if (
        not train_history.get("grad_norm_realized")
        and train_history.get("grad")
        and train_history.get("epoch")
    ):
        epochs = np.asarray(train_history["epoch"], dtype=float)
        if len(epochs) == 0:
            return

        grad_arr = np.asarray(train_history["grad"], dtype=float)
        ...
        with figure_context((8, 5), out_path):
            ...
            plt.xlabel("Epoch")
            plt.ylabel("L2 Norm")
            plt.title("Gradient Norms (Epoch Averages)")
            plt.grid(True, alpha=0.3)
            plt.legend()
    else:
        # Use new function instead
        save_gradient_diagnostics(train_history, out_path)


def save_gradient_norms_separate(train_history: dict, out_path: str):
    """Save separate gradient norm curves for KL and reconstruction terms. DEPRECATED - use save_gradient_diagnostics instead."""
    # For backward compatibility, fallback to old behavior if new keys not available
    if (
        not train_history.get("recon_grad_norm")
        and train_history.get("recon_grad")
        and train_history.get("kl_grad")
        and train_history.get("epoch")
    ):
        epochs = np.asarray(train_history["epoch"], dtype=float)
        if len(epochs) == 0:
        ...
        total_grad_arr = (
            np.asarray(train_history["grad"], dtype=float)
            if train_history.get("grad")
            else None
        )
        recon_grad_arr = np.asarray(train_history["recon_grad"], dtype=float)
        kl_grad_arr = np.asarray(train_history["kl_grad"], dtype=float)

        # Create figure and axes directly without figure_context
        fig, axs = plt.subplots(2, 2, figsize=(12, 8))

        # Top left: All gradient norms together
        axs[0, 0].plot(
            epochs,
            recon_grad_arr,
            "-o",
        ...
        axs[0, 0].set_title("All Gradient Norms")
        axs[0, 0].legend()
        axs[0, 0].grid(True, alpha=0.3)

        # Top right: Reconstruction gradient norm only
        axs[0, 1].plot(
            epochs,
            recon_grad_arr,
            "-o",
        ...
        axs[0, 1].set_title("Reconstruction Gradient Norm")
        axs[0, 1].legend()
        axs[0, 1].grid(True, alpha=0.3)

        # Bottom left: KL gradient norm only
        axs[1, 0].plot(
            epochs,
            kl_grad_arr,
            "-s",
        ...
        axs[1, 0].set_title("KL Gradient Norm")
        axs[1, 0].legend()
        axs[1, 0].grid(True, alpha=0.3)

        # Bottom right: Ratio of KL to Recon gradient norms
        ratio = kl_grad_arr / (
            recon_grad_arr + 1e-8
        ...
        axs[1, 1].axhline(
            y=1.0, color="gray", linestyle="--", alpha=0.7, label="Ratio = 1"
        )
        axs[1, 1].set_xlabel("Epoch")
        axs[1, 1].set_ylabel("Ratio")
        axs[1, 1].set_title("KL/Reconstruction Gradient Ratio")
        axs[1, 1].legend()
...
def save_gradient_contributions(train_history: dict, out_path: str):
    """Save gradient contribution analysis including dot products and cosine similarities. DEPRECATED - use save_gradient_diagnostics instead."""
    # Use new function instead
    save_gradient_diagnostics(train_history, out_path)


def save_loss_scatter(test_history: dict, out_path: str):
    """Save 2D scatter of BCE vs KL from test data colored by epoch."""
    if not test_history or not test_history.get("epoch"):
        return

    test_epochs = np.asarray(test_history["epoch"], dtype=float)
    recon_arr = np.asarray(test_history["recon"], dtype=float)
    kl_arr = np.asarray(test_history["kl"], dtype=float)

    if len(test_epochs) == 0:
        return

    with figure_context((8, 6), out_path):
        # Normalize epochs for color mapping (cool to warm)
        if len(test_epochs) > 1:
            epoch_normalized = (test_epochs - test_epochs.min()) / (
                test_epochs.max() - test_epochs.min()
        ...
        if len(test_epochs) > 1:
            ...
            points = np.column_stack([kl_arr, recon_arr])
            segments = []
            for i in range(len(points) - 1):
                segments.append([points[i], points[i + 1]])
        ...
        plt.ylabel("BCE (Reconstruction)")
        plt.title("Test: BCE vs KL (training path)")
...
def save_kl_per_dimension_bar(
    test_history: dict, out_path: str, active_threshold: float = 0.1
):
    """Save bar chart showing KL divergence for each latent dimension."""
    if not test_history.get("kl_per_dim") or not test_history.get("epoch"):
        return

    # Get the latest KL per dimension values
    kl_per_dim_history = test_history["kl_per_dim"]
    if not kl_per_dim_history:
        return

    latest_kl_per_dim = np.asarray(kl_per_dim_history[-1], dtype=float)
    latent_dim = len(latest_kl_per_dim)

    with figure_context((max(8, latent_dim * 0.6), 6), out_path):
        # Create bar chart
        ...
        plt.axhline(
            ...
            linestyle="--",
            alpha=0.7,
            label=f"Active threshold = {active_threshold}",
        )

        # Formatting
        plt.xlabel("Latent Dimension")
        plt.ylabel("KL Divergence")
        plt.title("KL Divergence per Dimension (Final Epoch)")
        plt.xticks(dims, [f"z{i + 1}" for i in dims])
        plt.grid(True, alpha=0.3, axis="y")
        plt.legend()

        # Add value labels on bars
        for i, (bar, val) in enumerate(zip(bars, latest_kl_per_dim)):
        ...
        plt.text(
            0.02,
            0.98,
            f"Active units: {active_units}/{latent_dim}",
...
def save_active_units_over_time(
    test_history: dict, out_path: str, active_threshold: float = 0.1
):
    """Save line plot showing number of active units over training epochs."""
    if not test_history.get("kl_per_dim") or not test_history.get("epoch"):
        return
    ...
    # Calculate active units for each epoch
    active_units_over_time = []
    total_kl_over_time = []

    for kl_per_dim in kl_per_dim_history:
        kl_array = np.asarray(kl_per_dim, dtype=float)
        active_units = np.sum(kl_array >= active_threshold)
        total_kl = np.sum(kl_array)
        active_units_over_time.append(active_units)
        total_kl_over_time.append(total_kl)

    active_units_over_time = np.array(active_units_over_time)
    total_kl_over_time = np.array(total_kl_over_time)
    latent_dim = len(kl_per_dim_history[0]) if kl_per_dim_history else 0

    # Create figure and axes directly without figure_context for dual y-axis
    fig, ax1 = plt.subplots(figsize=(10, 6))

    # Plot active units
    color1 = "tab:blue"
    ax1.set_xlabel("Epoch")
    ax1.set_ylabel("Active Units", color=color1)
    ax1.plot(
        epochs,
        active_units_over_time,
        "o-",
    ...
    ax1.set_ylim([0, latent_dim + 0.5])
    ax1.grid(True, alpha=0.3)

    # Add horizontal line for maximum possible active units
    ax1.axhline(
        y=latent_dim,
        color=color1,
        linestyle="--",
        alpha=0.5,
        label=f"Max units ({latent_dim})",
    )

    # Create second y-axis for total KL
    ax2 = ax1.twinx()
    color2 = "tab:red"
    ax2.set_ylabel("Total KL Divergence", color=color2)
    ax2.plot(
        epochs,
        total_kl_over_time,
        "s-",
    ...
    # Title and legend
    plt.title(f"Active Units Over Time (threshold = {active_threshold})")
    ...
    final_active = active_units_over_time[-1]
    final_total_kl = total_kl_over_time[-1]
    ax1.text(
        0.02,
        0.98,
        f"Final: {final_active}/{latent_dim} active\nTotal KL: {final_total_kl:.3f}",
...
def save_kl_dimension_heatmap(test_history: dict, out_path: str):
    """Save heatmap showing KL per dimension evolution over epochs."""
    if not test_history.get("kl_per_dim") or not test_history.get("epoch"):
        return
    ...
    # Convert to numpy array [n_epochs, n_dimensions]
    kl_matrix = np.array(
        [np.asarray(kl_per_dim, dtype=float) for kl_per_dim in kl_per_dim_history]
    )
    latent_dim = kl_matrix.shape[1]

    with figure_context((max(8, latent_dim * 0.8), 6), out_path):
        # Create heatmap
        im = plt.imshow(kl_matrix.T, aspect="auto", cmap="viridis", origin="lower")

        # Set ticks and labels
        if len(epochs) <= 20:
            # Show all epochs if not too many
            plt.xticks(range(len(epochs)), [f"{int(e)}" for e in epochs])
        else:
            # Show subset of epochs
            step = max(1, len(epochs) // 10)
            tick_indices = range(0, len(epochs), step)
            plt.xticks(tick_indices, [f"{int(epochs[i])}" for i in tick_indices])

        plt.yticks(range(latent_dim), [f"z{i + 1}" for i in range(latent_dim)])

        plt.xlabel("Epoch")
        plt.ylabel("Latent Dimension")
        plt.title("KL Divergence per Dimension Over Time")

        # Add colorbar
        cbar = plt.colorbar(im)
        cbar.set_label("KL Divergence")

        # Add text annotations for small dimensions
        if latent_dim <= 10 and len(epochs) <= 20:
